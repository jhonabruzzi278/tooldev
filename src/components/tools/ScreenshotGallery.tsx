...
export default function ScreenshotGallery({ toolId, toolName, totalImages }: ScreenshotGalleryProps) {
  ...
  const prev = useCallback(() => setCurrent((c) => (c - 1 + totalImages) % totalImages), [totalImages]);
  const next = useCallback(() => setCurrent((c) => (c + 1) % totalImages), [totalImages]);
