...
- [x] Construction Phase — **~92%** — código funcional, 131 tests pasando (110 originales + 21 nuevos de `favorites.ts`/`licenses.ts`/`utils.ts`), CI configurado (`.github/workflows/test.yml`), coverage medido y con umbral en CI (45% stmts / 30% branches / 30% funcs / 48% lines — no es el 80% objetivo de la política estándar, ver `testing/TEST_COVERAGE_REPORT.md`). Falta: subir coverage de componentes UI a 80%.
...
5. **README desactualizado:** El README afirma "64 herramientas, 6 categorías, 43 tests — todos pasando". La realidad medida es: **102 herramientas, 7 categorías, 110 tests con 1 fallando**. Los documentos generados usan los datos reales medidos.
